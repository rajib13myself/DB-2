...
void initialize_pde(double* u) {
    ...
    for (int i = 0; i < TGX; i++) {
        u[i * TGY] = 50.0;          // Bottom boundary
        u[i * TGY + TGY - 1] = 0.0; // Top boundary
    }
    for (int j = 0; j < TGY; j++) {
        u[j] = 50.0;               // Left boundary
...
void timestep_fd(double* u, double* un, FILE* file) {
    ...
    for (int i = 1; i < TGX - 1; i++) {
        for (int j = 1; j < TGY - 1; j++) {
            un[i * TGY + j] = u[i * TGY + j] + ALPHA * TS * (
                (u[(i+1) * TGY + j] - 2.0 * u[i * TGY + j] + u[(i-1) * TGY + j]) / (GSX * GSX) +
                (u[i * TGY + (j+1)] - 2.0 * u[i * TGY + j] + u[i * TGY + (j-1)]) / (GSY * GSY)
            );
        }
    }
