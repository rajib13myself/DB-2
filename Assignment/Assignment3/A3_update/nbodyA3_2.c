...
int main() {
	...
	FILE *read_file = fopen("Input_data.txt", "r");
	if(read_file == NULL) {
	...
	for(int i = 0; i < N; i++) {
		fscanf(read_file, "%lf %lf %lf %lf %lf %lf", &particles[i].mass, &particles[i].position.x, &particles[i].position.y, &particles[i].velocity.x, &particles[i].velocity.y, &particles[i].brightness);
	}
	fclose(read_file);
